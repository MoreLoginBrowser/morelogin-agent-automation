...
### 2.3 Recommended pinned dependency versions (2026 baseline)

To reduce breakage from future API changes, pin critical dependencies explicitly in `package.json`:
...
```json
{
  ...
  "type": "module",
  "scripts": {
    "workflow:a": "bash ./scripts/workflow-a.sh"
  },
  "dependencies": {
    "dotenv": "16.4.7"
  },
...
If you use npm install commands, prefer exact versions:

```bash
npm install -E dotenv@16.4.7
npm install -D -E @playwright/test@1.51.1 tsx@4.19.2
```
...
ML_DIR="$HOME/.openclaw/workspace/skills/morelogin"
...
### 7.1 Do not publish sensitive identifiers directly

Before uploading to GitHub, avoid hardcoding any potentially sensitive values in scripts, including:

- `envId` linked to your production browser profiles
- API tokens or keys
- Proxy credentials / account credentials

Use environment variables or `.env` files instead of literal values:
...
Also review commit diffs before push:
